...
class AplicativoJogodeDados extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      title: 'Jodo de Dados', //Título que aparece no gerenciador
...
class _EstadoTelaConfiguracaoJogadores
  ...
  // FormState é o estado interno desse formulário, é a parte que sabe oq está digitado e consegue validar os campos
  final _chaveFormulario = GlobalKey<FormState>();
  // Controladores para pegar o texto digitado nos campos
  final TextEditingController _controllerJogador1 = TextEditingController();
  final TextEditingController _controllerJogador2 = TextEditingController();
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                controller: _controllerJogador1, // liga o input ao controlador
              ...
              TextFormField(
                controller: _controllerJogador2, // liga o input ao controlador
              ...
              ElevatedButton(
                onPressed: () {
                  // Checar se o formulário está válido(se os campos fora, preenchidos)
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        ...
                        builder: (context) => TelaJogodeDados(
                          nomeJogador1: _controllerJogador1.text,
                          nomeJogador2: _controllerJogador2.text,
                        ),
                      ),
                    );
                  }
...
class TelaJogodeDados extends StatefulWidget {
  // Variáveis finais que armazena os nomes recebidos da tela anterior
  ...
  @override
  // Ei tio flutter, quando essa tela for criada, use essa classe chamada_EsttadoTelaJogoDados
...
class _EstadoTelaJogoDeDados extends State<TelaJogodeDados> {
  ...
  // Lógica da puntuação: verifica combinações para aplicar os multiplicadores.
  int _calcularPontuacao(List<int> lancametos) {
    // reduce percorre toda a lista somando tudo
    final soma = lancametos.reduce((a, b) => a + b);
    // [4,4,1] > 4 + 4 = 8 > 8 + 1 = 9 > soma = 9
    final valoresUnicos = lancametos.toSet().length;
  ...
  void _lancarDados() {
    // eu uso o sublinhado _ significa que ela é privada, só pode ser usada
    // dentro dessa classe
  ...
  Widget _construirColunaJogador(String nome, List<int> lancamentos) {
    return Expanded(
      // Pega todo o espaço disponível dentro de um row ou column
      child: Column(
        children: [
          Text(
            nome,
